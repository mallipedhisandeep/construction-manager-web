...
function LoginInner() {
  ...
  const signInWithGoogle = async () => {
    setLoading(true)
    setError('')
    // Use the current window origin so it ALWAYS redirects back to 
    // the correct domain (Vercel preview, production, custom domain, etc.)
  ...
  return (
    ...
    >
      {/* Dark gradient overlay — heavier at bottom so button stands out */}
      ...
      <div className="relative z-10 w-full max-w-sm px-5">
        ...
        {error && (
          <div className="mb-4 px-4 py-3 rounded-2xl text-sm text-center"
            style={{background:'rgba(185,28,28,0.4)',border:'1px solid rgba(239,68,68,0.5)',color:'#fca5a5'}}>
            {error}
          </div>
        )}

        {/* Google sign-in button — single, white, clean */}
        <button
          onClick={signInWithGoogle}
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-bold rounded-2xl py-4 text-base transition active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed select-none"
          style={{
        ...
        {/* Telugu label */}
        <p className="text-center text-xs mt-4 font-medium select-none"
