...
function ReportsPage() {
  ...
  const handlePrint = () => {
    ...
    // FIX: proper string interpolation — no curly braces inside template literal strings
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <table>
            <thead><tr><th>${te?'సైటు':'Site'}</th><th>${te?'స్థితి':'Status'}</th><th>${te?'బడ్జెట్':'Budget'}</th><th>${te?'స్వీకరించబడింది':'Received'}</th><th>${te?'ఖర్చు':'Spent'}</th><th>P/L</th></tr></thead>
            <tbody>
          ...
          <table>
            <thead><tr><th>${te?'పేరు':'Worker'}</th><th>${te?'రోజులు':'Days'}</th><th>${te?'సంపాదించినది':'Earned'}</th><th>${te?'అడ్వాన్స్':'Advance'}</th><th>${te?'బాకీ':'Balance'}</th></tr></thead>
            <tbody>
          ...
          <table>
            <thead><tr><th>${te?'వర్గం':'Category'}</th><th>${te?'బకాయి మొత్తం':'Amount Due'}</th></tr></thead>
            <tbody>
              <tr><td>${te?'కార్మికుల వేతనాలు':'Worker Wages Due'}</td><td class="red">₹${outstanding.workers.toFixed(0)}</td></tr>
              <tr><td>${te?'సరఫరాదారు బిల్లులు':'Supplier Bills Due'}</td><td class="red">₹${outstanding.suppliers.toFixed(0)}</td></tr>
              <tr><td>${te?'ప్రైవేట్ కాంట్రాక్టర్లు':'Private Contractors'}</td><td class="red">₹${outstanding.privateWorkers.toFixed(0)}</td></tr>
            </tbody>
          </table>
        </body>
      </html>
  ...
  // Tab labels — must be plain strings, not JSX expressions embedded in data arrays
      ...
      <div className="page">
        <div className="page-header">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-xl font-black dark:text-slate-100 text-gray-800">📊 {te ? 'నివేదికలు' : 'Reports'}</h1>
            <button onClick={handlePrint}
              className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 text-gray-700 text-sm font-semibold px-3 py-2 rounded-xl transition">
              🖨️ {te ? 'PDF డౌన్లోడ్' : 'Export PDF'}
            </button>
          </div>
              ...
              {loading ? (
                ...
                <>
                  <div className={`rounded-2xl p-5 text-center ${overview.netPL>=0?'bg-gradient-to-br from-green-500 to-emerald-600':'bg-gradient-to-br from-red-500 to-red-600'}`}>
                    ...
                    {/* FIX: use plain ternary, not {te?'...'} inside JSX string context */}
                    <p className="text-white/80 text-sm mt-1">
                      {overview.netPL >= 0
                        ? (te ? 'లాభంలో ఉన్నారు 🎉' : 'You are in profit 🎉')
                        : (te ? 'నష్టంలో ఉన్నారు ⚠️' : 'You are at a loss ⚠️')}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="card p-4">
                    <p className="font-bold dark:text-slate-200 text-gray-700 mb-3">{te?'ఖర్చుల వివరాలు':'Expense Breakdown'}</p>
                    {([
                      [te?'కార్మికుల వేతనాలు':'Worker Wages', overview.totalWorkerSpend],
                      [te?'వస్తువులు & సామగ్రి':'Goods & Materials', overview.totalGoodsSpend],
          ...
          {tab==='sites' && (
            <div className="space-y-3">
              {loading ? [0,1,2].map(i=><SkeletonCard key={i} rows={3}/>) :
               siteReports.length===0 ? <div className="text-center py-16 dark:text-slate-500 text-gray-400">{te?'సైటు డేటా లేదు':'No site data'}</div>
               : siteReports.map(s=>{
                  ...
                  <div key={s.id} className="card p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-right">
                        <p className={`font-black ${netPL>=0?'text-green-600':'text-red-500'}`}>₹{Math.abs(netPL).toFixed(0)}</p>
                        {/* FIX: proper ternary, not {te?'...'} as JSX object */}
                        <p className="text-xs dark:text-slate-500 text-gray-400">
                          {netPL >= 0 ? (te ? 'లాభం' : 'Profit') : (te ? 'నష్టం' : 'Loss')}
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-center text-xs">
                      <div className="bg-blue-50 rounded-lg p-2"><p className="font-bold text-blue-600">₹{(s.budget/100000).toFixed(1)}L</p><p className="text-blue-400">{te?'బడ్జెట్':'Budget'}</p></div>
                      <div className="bg-green-50 rounded-lg p-2"><p className="font-bold text-green-600">₹{s.received.toFixed(0)}</p><p className="text-green-400">{te?'స్వీకరించబడింది':'Received'}</p></div>
                      <div className="bg-red-50 rounded-lg p-2"><p className="font-bold text-red-500">₹{spend.toFixed(0)}</p><p className="text-red-400">{te?'ఖర్చు':'Spent'}</p></div>
                    </div>
                    {s.budget>0 && (
                      <div className="mt-3">
                ...
                <div key={w.id} className="card p-4 flex items-center gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] dark:text-slate-500 text-gray-400">
                      {w.balance > 0 ? (te ? 'ఇవ్వాలి' : 'Owe') : (te ? 'అడ్వాన్స్' : 'Advance')}
                    </p>
                  </div>
                </div>
              ))}
                ...
                <>
                  <div className="card p-5">
                    ...
                    <div className="mt-4 pt-3 border-t dark:border-slate-700 border-gray-100 flex justify-between">
                      <span className="font-black dark:text-slate-200 text-gray-700">{te?'మొత్తం బకాయి':'Total Outstanding'}</span>
                  ...
                  <div className="card p-4 flex items-center gap-3">
                    ...
                    <div><p className="font-bold dark:text-slate-100 text-gray-800">{outstanding.sitesPending} {te?'చురుకైన సైట్లు':'Active Sites'}</p><p className="text-sm dark:text-slate-400 text-gray-500">{te?'నిర్మాణంలో ఉన్నవి':'Currently under construction'}</p></div>
                  </div>
                </>
              )}
            </div>
