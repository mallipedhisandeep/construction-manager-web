...
// Pad number to 2 digits — defined outside component so no re-render loop
...
function AttendancePage() {
  ...
  // ── Load marked days for current month (for sidebar colouring) ───────────
  const loadMonthMarks = useCallback(async () => {
    ...
    data.forEach(a => {
      byDay[a.date_key] = byDay[a.date_key] ?? []
      byDay[a.date_key].push(a.attendance_type)
    })
  ...
  // ── Open attendance modal for a worker ───────────────────────────────────
  ...
  const saveOne = async () => {
    ...
    try {
      ...
      const payload = {
        worker_id: modal.id!, date_key: dKey,
  ...
  // ── Open monthly summary for a worker ────────────────────────────────────
  ...
  workers.forEach(w => {
    grouped[w.work_type] = grouped[w.work_type] ?? []
    grouped[w.work_type].push(w)
  })
    ...
    <div className="page" style={{ display:'flex', flexDirection:'column' }}>
      ...
      <div className="page-header">
        {view === 'summary' ? (
          <div className="flex items-center gap-3">
            <button onClick={()=>setView('day')} className="text-amber-500 font-bold text-sm">← {ts(lang,'back')}</button>
            ...
            {sumLoading && <div className="w-4 h-4 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"/>}
          </div>
        ) : (
          <>
            {/* Year + month strip */}
          ...
          {sumLoading ? (
            ...
            <>
              ...
              <div className="card p-4 mb-4 text-center">
                ...
                <p className="text-3xl font-black" style={{color: sumBalance>0?'#16a34a':sumBalance<0?'#dc2626':'rgb(var(--muted))'}}>
                  ₹{Math.abs(sumBalance)}
                </p>
      ...
      {view === 'day' && (
        <div className="flex flex-1 overflow-hidden" style={{ height:'calc(100vh - 148px)' }}>
            ...
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
              ...
              return (
                <button key={d} onClick={() => setDay(d)}
                  ...
                  {s.dot && (
                    <span className={`w-1.5 h-1.5 rounded-full mt-0.5 ${s.dot}`}/>
                  )}
                </button>
              )
            })}
          ...
          {/* RIGHT — worker list for selected day */}
          <div className="flex-1 overflow-y-auto">
            {/* Site selector strip */}
            {sites.length > 0 && (
              <div className="px-3 pt-3 pb-2 border-b" style={{borderColor:'rgb(var(--border))'}}>
                        ...
                        <div key={w.id} className="card mb-2 flex items-center gap-2.5 p-2.5">
                            ...
                            {att ? (
                              <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                                <span className="text-[11px] font-bold px-2 py-0.5 rounded-full text-white"
                                  style={{background: col ?? '#888', fontSize:10}}>
                          ...
                          <div className="flex gap-1 flex-shrink-0">
                            ...
                            <button onClick={()=>openModal(w)}
                              className="p-1.5 rounded-lg text-sm transition"
                              style={{color: att?'#d48c28':'#16a34a', background: att?'rgba(212,140,40,0.1)':'rgba(22,163,74,0.1)'}}>
                              {att ? '✏️' : '➕'}
                            </button>
                          </div>
                        </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center"
          ...
          <div className="w-full max-w-lg rounded-t-3xl shadow-2xl pb-safe"
