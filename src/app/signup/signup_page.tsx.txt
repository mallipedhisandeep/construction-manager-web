...
export default function SignupPage() {
  ...
  const sendOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // signUp triggers the OTP email when email confirmation is ON in Supabase
      const { data, error: err } = await supabase.auth.signUp({ email, password })
      if (err) { setError(err.message); return }
      // If session returned immediately (email confirm OFF) → go straight to app
  ...
  const verifyOtp = async () => {
    const code = otp.join('')
    if (code.length < 8) { setError('Please enter all 8 digits'); return }
    ...
    try {
      ...
      if (err) { setError('Wrong OTP or it has expired. Try again.'); return }
      router.push('/')
    } catch(e: unknown) {
  ...
  const handleOtpPaste = (e: React.ClipboardEvent) => {
    ...
    if (pasted.length > 0) {
      const next = [...otp]
      pasted.split('').forEach((d, i) => { if (i < 8) next[i] = d })
      setOtp(next)
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          {step === 'form' && (
            <>
              ...
              <p className="text-gray-400 text-sm mb-5">An 8-digit OTP will be sent to your email</p>
              <form onSubmit={sendOtp} className="space-y-4">
                ...
                {error && (
                  <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl px-3 py-2.5 text-sm">{error}</div>
                )}
          ...
          {step === 'otp' && (
            <>
              ...
              <div className="flex justify-center gap-2 mb-6" onPaste={handleOtpPaste}>
                {otp.map((digit, i) => (
                  <input
                    ...
                    className={`w-9 h-11 text-center text-lg font-black border-2 rounded-xl focus:outline-none transition
              ...
              {error && (
                <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl px-3 py-2.5 text-sm mb-4 text-center">{error}</div>
              )}

              <button onClick={verifyOtp} disabled={loading || otp.join('').length < 8}
              ...
              <div className="text-center">
                {resendTimer > 0 ? (
                  <p className="text-sm text-gray-400">Resend OTP in <span className="font-bold text-orange-500">{resendTimer}s</span></p>
                ) : (
