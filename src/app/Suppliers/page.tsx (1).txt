...
function SuppliersPage() {
  ...
  const saveGoods = async () => {
    ...
    setSaving(true)
    try {
      const { error } = await supabase.from('supplier_goods').insert({ ...gForm, supplier_id:selected.id })
      if (error) throw error
  ...
  const savePayment = async () => {
    ...
    try {
      ...
      if (error) throw error
      // Reload detail and list (balance changes)
        ...
        <>
          <div className="page-header">
            <div className="flex items-center justify-between">
              ...
              <button onClick={()=>{ setSForm({}); setModal('supplier') }} className="btn-primary btn-sm">+ Add Supplier</button>
            </div>
          </div>
