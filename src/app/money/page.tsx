...
function MoneyPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      // FIX M2: filter all queries by user_id
      ...
      const [
        ...
        supabase.from('private_worker_payments').select('amount,direction').eq('user_id',userId),
        supabase.from('private_work').select('price_charged,amount_paid').eq('user_id',userId),
